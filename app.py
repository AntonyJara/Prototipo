...
def set_cell_border(cell, **kwargs):
    ...
    for edge in ('top', 'left', 'bottom', 'right', 'inside_h', 'inside_v'):
        border_name = 'w:' + edge
...
def crear_documento_word(ciclo, area, tema, contenido):
    ...
    # PÁGINA 1: TABLA DE COMPETENCIAS
    ...
    tabla1 = doc.add_table(rows=1, cols=5)
    ...
    titulo2 = doc.add_heading('INSTRUMENTO DE EVALUACIÓN', 1)
    titulo2.alignment = WD_ALIGN_PARAGRAPH.CENTER

    subtitulo = doc.add_heading('LISTA DE COTEJOS', 2)
    ...
    desc.add_run('Competencia: ').bold = True
    desc.add_run(f'{contenido.get("competencia", "N/A")}')

    desc2 = doc.add_paragraph()
    desc2.add_run('Criterios de evaluación: ').bold = True
    criterios_text = " | ".join(contenido.get('criterios', []))
    desc2.add_run(criterios_text)
